...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error = hidden_error.transpose(1, 0)

        # hidden_error_term.shape: 1 X 2
        hidden_error_term = hidden_error * hidden_outputs * (1 - hidden_outputs)
        
        # delta_weights_i_h.shape: 3 X 2
        # delta_weights_h_o.shape: 2 X 1

        # X.shape: 1 X 3

        # Weight step (input to hidden) 3 X 1 * 1 X 2 = 3 X 2
        delta_weights_i_h += hidden_error_term * X[:,None] #None

        # Weight step (hidden to output)
        delta_weights_h_o += np.dot(output_error_term[0], hidden_outputs[:,None]) #None
